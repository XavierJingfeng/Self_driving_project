...
#include "an_dynamic_planner/Node.h"
...
public:
	...
	*/
	...
	int lane_shift;
	int sect_shift;
	std::vector<std::vector<double>> inner_pts;  // [x, y, theta, t]
	double total_traverse_time;
	motion_primitive(unsigned id, double length, double width, double velocity); // 1. use bezier curve
	~motion_primitive();
